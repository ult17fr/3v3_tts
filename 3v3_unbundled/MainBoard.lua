...
function MainBoard.recruitSardaukarCommanderFromAssemblyHall(color)
    if MainBoard.sardaukarCommanders.sc_AssemblyHall ~= nil then
        ...
        if Commander.isTeamShaddam(color) and Commander.isCommander(color) then
                ...
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
                            ...
                            broadcastToAll(I18N("recruitSardaukarCommanderAssemblyHall"), normalOrAllyColor)
                            Combat.recruitSardaukarCommander(normalOrAllyColor)
        ...
        elseif Commander.isTeamShaddam(color) and not Commander.isCommander(color) then
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
        ...
        elseif Commander.isTeamMuadDib(color) then
            if PlayBoard.getResource(color, "spice"):get() >= 2 then
                Dialog.showYesOrNoDialog(color, I18N("killSardaukarCommanderConfirm"), continuation, function (confirmed)
                    if confirmed then
                        local playBoard = PlayBoard.getPlayBoard(color)
                        broadcastToAll(I18N("killSardaukarCommander"), color)
                        Helper.onceFramesPassed(5).doAfter(function ()
                            playBoard.leader.resources(color, "spice", -2)
                            playBoard.leader.drawIntrigues(color, 1)
                            MainBoard.sardaukarCommanders.sc_AssemblyHall.destruct()
                            MainBoard.sardaukarCommanders.sc_AssemblyHall = nil
                        end)
                    end
                ...
                Dialog.broadcastToColor(I18N('noSpice'), color, "Purple")
            end
        end
    end
end
            ...
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
                            local playBoard = PlayBoard.getPlayBoard(color)
                            broadcastToAll(I18N("recruitSardaukarCommanderGatherSupport"), normalOrAllyColor)
                            Combat.recruitSardaukarCommander(normalOrAllyColor)
                            Helper.onceFramesPassed(5).doAfter(function ()
                                playBoard.leader.resources(color, "solari", -1)
            ...
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
                            local playBoard = PlayBoard.getPlayBoard(color)
                            broadcastToAll(I18N("recruitSardaukarCommanderHighCouncil"), normalOrAllyColor)
                            Combat.recruitSardaukarCommander(normalOrAllyColor)
                            Helper.onceFramesPassed(5).doAfter(function ()
                                playBoard.leader.resources(color, "solari", -1)
            ...
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
                            local playBoard = PlayBoard.getPlayBoard(color)
                            broadcastToAll(I18N("recruitSardaukarCommanderSardaukar"), normalOrAllyColor)
                            Combat.recruitSardaukarCommander(normalOrAllyColor)
                            Helper.onceFramesPassed(5).doAfter(function ()
                                playBoard.leader.resources(color, "solari", -1)
            ...
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
                            local playBoard = PlayBoard.getPlayBoard(color)
                            broadcastToAll(I18N("recruitSardaukarCommanderDutifulService"), normalOrAllyColor)
                            Combat.recruitSardaukarCommander(normalOrAllyColor)
                            Helper.onceFramesPassed(5).doAfter(function ()
                                playBoard.leader.resources(color, "solari", -1)
            ...
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderConfirmOneSolari"), continuation, function (confirmed)
                        if confirmed then
                            local playBoard = PlayBoard.getPlayBoard(color)
                            broadcastToAll(I18N("recruitSardaukarCommanderDeliverSupplies"), normalOrAllyColor)
                            Combat.recruitSardaukarCommander(normalOrAllyColor)
                            Helper.onceFramesPassed(5).doAfter(function ()
                                playBoard.leader.resources(color, "solari", -1)
    ...
    if MainBoard.sardaukarCommanders.sc_Standard ~= nil then
        ...
        if Commander.isTeamShaddam(color) and Commander.isCommander(color) then
            Dialog.showYesOrNoDialog(color, I18N("recruitSardaukarCommanderFreeConfirm"), continuation, function (confirmed)
                if confirmed then
                    local playBoard = PlayBoard.getPlayBoard(color)
                    broadcastToAll(I18N("recruitSardaukarCommanderStandard"), normalOrAllyColor)
                    Combat.recruitSardaukarCommander(normalOrAllyColor)
                    Helper.onceFramesPassed(5).doAfter(function ()
    ...
    if MainBoard.sardaukarCommanders.sc_NoPopup ~= nil then
        ...
        if Commander.isTeamShaddam(color) and Commander.isCommander(color) then
            if PlayBoard.hasTech(color, "bl_SardaukarHighCommand") then
                if PlayBoard.getResource(color, "solari"):get() >= 1 then
                    local playBoard = PlayBoard.getPlayBoard(color)
                    broadcastToAll(I18N("recruitSardaukarCommanderNoPopup"), normalOrAllyColor)
                    Combat.recruitSardaukarCommander(normalOrAllyColor)
                    Helper.onceFramesPassed(5).doAfter(function ()
                        playBoard.leader.resources(color, "solari", -1)
                    end)
...
function MainBoard._goMilitarySupport(color, leader, continuation)
    ...
    elseif MainBoard._checkGenericAccess(color, leader, { spice = 2 }) then
        continuation.run(function ()
...
function MainBoard._goHighCouncil(color, leader, continuation)
    local cost = (
        PlayBoard.getLeader(color).name == "letoAtreides" and PlayBoard.hasTech(color, "bl_NavigationChamber") and 3
        or PlayBoard.getLeader(color).name == "letoAtreides" and 4
        or PlayBoard.hasTech(color, "bl_NavigationChamber") and 4
        or 5
    )

    if MainBoard._checkGenericAccess(color, leader, { solari = cost }) then
        continuation.run(function ()
            leader.resources(color, "solari", -cost)
...
function MainBoard._goImperialPrivilege(color, leader, continuation)
    local cost = (
        PlayBoard.getLeader(color).name == "letoAtreides" and PlayBoard.hasTech(color, "bl_NavigationChamber") and 1
        or PlayBoard.getLeader(color).name == "letoAtreides" and 2
        or PlayBoard.hasTech(color, "bl_NavigationChamber") and 2
        or 3
    )

    if MainBoard._checkGenericAccess(color, leader, { solari = cost, friendship = "emperor" }) then
        continuation.run(function ()
            leader.resources(color, "solari", -cost)
            leader.drawImperiumCards(color, 1)
...
function MainBoard._getSwordmasterCost()
    ...
    local color = TurnControl.players[TurnControl.currentPlayerLuaIndex]
    local leader = PlayBoard.getLeader(color).name

    if PlayBoard.hasTech(color, "bl_NavigationChamber") then
        if leader == "bl_Duncan" then
            return firstAccess and 5 or 3
        elseif leader == "letoAtreides" then
...
function MainBoard._goGatherSupport(color, leader, continuation)
    -- bloodlines tech navigation chamber
    if PlayBoard.getLeader(color).name == "letoAtreides" then
        if PlayBoard.hasTech(color, "bl_NavigationChamber") then
            MainBoard._goGatherSupport_WithWater(color, leader, continuation)
        elseif MainBoard._hasResource(leader, color, "solari", 1) then
            local options = {
                I18N("noWaterOption"),
    ...
    elseif PlayBoard.hasTech(color, "bl_NavigationChamber") then
...
function MainBoard._goDeepDesert(color, leader, continuation)
    ...
    if Commander.isCommander(color) and PlayBoard.hasMakerHook(normalOrAllyColor) then
        if PlayBoard.hasMakerHook(normalOrAllyColor) and (not MainBoard.shieldWallIsStanding() or not Combat.isCurrentConflictBehindTheWall()) then
            local options = {
    ...
    if Commander.isCommander(color) and PlayBoard.hasMakerHook(normalOrAllyColor) then
        if PlayBoard.hasMakerHook(normalOrAllyColor) and (not MainBoard.shieldWallIsStanding() or not Combat.isCurrentConflictBehindTheWall()) then
            MainBoard._anySpiceSpace(color, leader, 3, 0, MainBoard.spiceBonuses.deepDesert, continuation, function ()
                Combat.callSandworm(normalOrAllyColor, 2)
            end)
        else
